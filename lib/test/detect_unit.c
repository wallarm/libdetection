...
Tsqli_var_start_with_num(void)
{
    ...
    CU_ASSERT_EQUAL(
        detect_add_data(detect,
                      STR_LEN_ARGS("SELECT 1 FROM schema.1table_name"), true),
        0);
    CU_ASSERT_EQUAL(detect_has_attack(detect, &attack_types), 1);
    CU_ASSERT_EQUAL(detect_stop(detect), 0);
    CU_ASSERT_EQUAL(detect_start(detect), 0);
    CU_ASSERT_EQUAL(
        detect_add_data(detect, STR_LEN_ARGS("SELECT 1eUNION SELECT 1"), true),
        0);
    CU_ASSERT_EQUAL(detect_has_attack(detect, &attack_types), 1);
    CU_ASSERT_EQUAL(detect_stop(detect), 0);
    CU_ASSERT_EQUAL(detect_start(detect), 0);
    CU_ASSERT_EQUAL(
        detect_add_data(detect, STR_LEN_ARGS("SELECT 1e1UNION SELECT 1"), true),
        0);
    CU_ASSERT_EQUAL(detect_has_attack(detect, &attack_types), 1);
    CU_ASSERT_EQUAL(detect_stop(detect), 0);
    CU_ASSERT_EQUAL(detect_close(detect), 0);
