...
#define s_type_checks_(typename, has_attack, ...)                                           \
    s_type_checks(                                                                          \
        typename, (const struct detect_str[]){__VA_ARGS__},                                 \
        sizeof((const struct detect_str[]){__VA_ARGS__}) / sizeof(const struct detect_str), \
        has_attack)
...
s_type_checks(const char *typename, const struct detect_str *tests, size_t ntests, bool has_attack)
{
    ...
    for (size_t i = 0; i != ntests; i++) {
        ...
        CU_ASSERT_EQUAL(detect_add_data(detect, tests[i].str, tests[i].len, true), 0);
...
Tsqli_rce(void)
{
    ...
    CU_ASSERT_PTR_NOT_NULL_FATAL(detect = detect_open("sqli"));
    CU_ASSERT_EQUAL(detect_start(detect), 0);
    CU_ASSERT_EQUAL(detect_add_data(detect, STR_LEN_ARGS("UP"), true), 0);
    CU_ASSERT_EQUAL(detect_stop(detect), 0);
    CU_ASSERT_EQUAL(detect_start(detect), 0);
    CU_ASSERT_EQUAL(detect_add_data(detect, STR_LEN_ARGS("SELECT SELECT"), true), 0);
    CU_ASSERT_EQUAL(detect_stop(detect), 0);
    printf("Next!\n");
    CU_ASSERT_EQUAL(detect_start(detect), 0);
    CU_ASSERT_EQUAL(detect_add_data(detect, STR_LEN_ARGS("10 + 11 + 12 + 13"), true), 0);
    CU_ASSERT_EQUAL(detect_stop(detect), 0);
    CU_ASSERT_EQUAL(detect_close(detect), 0);
}
...
Tsqli_string(void)
{
    s_sqli_attacks({CSTR_LEN("SELECT _latin1 'str' COLLATE latin1_german2_ci a, n'str' b, "
                             "x'str' c, _utf8'str' d")});
}
...
Tsqli_delete(void)
{
    s_sqli_attacks({CSTR_LEN("DELETE LOW_PRIORITY "
...
Tsqli_buf(void)
{
    s_sqli_attacks({CSTR_LEN("SELECT 0x4445434C415245204054207661726368"
...
Tbash_comment(void)
{
    s_bash_not_attacks({CSTR_LEN("#ls -a")}, );
}
...
main(void)
{
    ...
    if ((sum = CU_get_run_summary()) == NULL || sum->nSuitesFailed || sum->nTestsFailed
        || sum->nAssertsFailed) {
        CU_cleanup_registry();
        return (EXIT_FAILURE);
    }
